    ...
    if(lexer->tk == LEX_ID)
    {
        ...
        while(lexer->tk == '(' || lexer->tk == '.' || lexer->tk == '[')
        {
            if(lexer->tk == '(')
            {
                ...
                std::vector<CSyntaxExpression*> args = functionCall();
...
CSyntaxStatement* CScriptSyntaxTree::block()
{
    ...
    while(lexer->tk && lexer->tk != '}')
    {
        ...
        if(seq)
        {
            ...
            delete seq;
            RANGED_FOR(std::vector<CSyntaxNode*>,stmt2,statements)
//            for(auto& stmt2 : statements)
                stmts = new CSyntaxSequence(stmts, *stmt2_);
        	}
        }
        else
...
CSyntaxFunction::~CSyntaxFunction()
{
    if(name)
        delete name;
    RANGED_FOR(std::vector<CSyntaxID*>,arg,arguments)
...
{
	RANGED_FOR(std::vector<CSyntaxExpression*>,arg,actuals)
